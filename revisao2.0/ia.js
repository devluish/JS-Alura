// 🧠 6. Sistema de inteligência artificial simples
// Crie uma função que recebe uma pergunta como string e responde com base nas palavras-chave:
// Se contiver "tempo" → "Hoje está ensolarado!"
// Se contiver "nome" → "Meu nome é ChatBot JS"
// Se não reconhecer → "Desculpe, não entendi sua pergunta."


const tempo = "Hoje está ensolarado";
const nome = "Meu nome é ChatBot JS";
const erro = "Desculpe, não entendi sua pergunta";

const chatbot = ("") => {
    const previsaoTempo = tempo;
    const chatbotNome = nome;
    const mensagemErro = erro;
    
    
}

chatbot();
chatbot(nome);
chatbot(erro);